...
#### 프로젝트 2.2 : 5일 이동평균 계산 및 결측치 처리
: 종가를 기준으로 암호화폐 마다 5일 이동 평균을 계산합니다.
결측치는 해당 날짜의 종가로 대체합니다.

#### 프로젝트 2.3 : 프로젝트(2.1+2.2) 데이터의 시각화
...
- **객체 지향 프로그래밍(OOP)**
: API 데이터 수집(class UpbitAPI) / 분석&계산(class Analyzer_Upbit) / 유틸함수(basic_func.py ) / 그래프(graph.py) / 1번 프로젝트(p01.py) / 2번 프로젝트(p02.py) / 3번 프로젝트(p03.py) / SQLite DB(Sqlite_Upbit.py) 전체 프로그램 실행(main) 으로 각각 기능 분리
- **모듈화**를 통해 각 클래스의 역할을 설정하고, 그에 맞게 메서드 분리하는 설계
...
- 객체 지향 프로그래밍(OOP) 설계
- 모듈화를 통한 역할 분리
- SQLite를 이용하여 데이터베이스를 생성하고, 데이터를 저장하고, 다시 불러오는 방법
...
| 클래스 / 함수 | 역할 |
|---|---|
| `class Sqlite_Upbit` | SQLite DB를 생성하고 데이터를 저장하거나 불러오는 기능을 제공합니다. |
| `__init__(dataframe=None)` | 저장할 DataFrame을 초기화합니다. (`None`으로 설정하여 데이터 없이도 객체를 생성할 수 있습니다.) |
| `create_database()` | `crypto_data.db` 데이터베이스에 연결하고 db파일이 없으면 생성합니다. |
| `save_to_database()` | 필요한 컬럼만 선택하여 `crypto_ohlcv` 테이블에 저장하고, 저장된 행의 개수를 반환합니다. |
| `load_from_database()` | `crypto_ohlcv` 테이블의 데이터를 조회하여 DataFrame으로 반환합니다. |
